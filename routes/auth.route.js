...
authRoute.post('/login', (req, res) => {
  ...
  UserModel.findOne(req.body)
    .then((data) => {
      ...
      // user exist in DB
      let token = jwt.sign({ data }, process.env.JWT_SECRET_KEY); // TOKEN ******************************************************
      //console.log(token);
...
authRoute.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      newUser.save();
      const activationMailHtml = mailTemplates.htmlActivation(
        `${process.env.ACTIVATION_MAIL_URL}/${newUser?._id}`
      ); //Edit when you host

      sendMail(newUser?.email, 'Activation Account', activationMailHtml)
        .then(() => res.send('User registered.'))
        .catch((error) => res.status(415).send(error));
    } catch (e) {
      res
        .status(416)
        .send(`Error while creating new user: ${req.body.username}`);
    }
  }
);
